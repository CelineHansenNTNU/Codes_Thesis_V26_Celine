function out = SF_VB_70_30_MultiSugarPLSR_MeanSpectra(trainRootDir, hplcExcelPath, predictRootDir, opts)
% Shake flask multi-response PLSR model using:
...
%   - optional application of locked model to exp1RootDir
%
% Celine Hansen + merged from LOGO + Venetian blinds setup + Copilot
    ...
    msgs{end+1} = sprintf('Exp2 Raman root:   %s\n', trainRootDir);
    ...
    [X_exp, Y_exp, G_exp, keys_exp, timeNum_exp, timeLabel_exp, commonShift, nRep_exp, XrawMean_exp] = ...
    ...
    % PART B: 70/30 SPLIT ON SF/T SAMPLES
    ...
    % Optional: keep only one selected flask in predictRootDir
    if ~isempty(opts.wantedExp1Flask)
        keepPred = ismember(G_exp_rep, opts.wantedExp1Flask);

        X_exp_rep = X_exp_rep(keepPred,:);
        G_exp_rep = G_exp_rep(keepPred);
        keys_exp_rep = keys_exp_rep(keepPred);
        timeNum_exp_rep = timeNum_exp_rep(keepPred);
        timeLabel_exp_rep = timeLabel_exp_rep(keepPred);
        nRep_exp_rep = nRep_exp_rep(keepPred);
        XrawMean_exp_rep = XrawMean_exp_rep(keepPred,:);

        if opts.evaluateExternalRMSEP
            Yrep_all = Yrep_all(keepPred,:);
        end

        if isempty(X_exp_rep)
            warning('No samples left after filtering predictRootDir to wanted flask(s).');
        end
    end
    ...
    if opts.saveMat
        save('SF_VB_70_30_MultiPLSR_SF8_MS_withSNV_maxLV20.mat', 'out');
    end
end
...
function [XmeanProc, Y, G, keysOut, timeNum, timeLabel, commonShift, nRep, XmeanRaw] = build_mean_dataset_from_root_local(rootDir, hplcMap, opts)
    ...
    for i = 1:numel(keysUniqueS)
        ...
        % keep only the last N replicates
        %Nkeep = 60;
        %fileInfo = cellfun(@(f) dir(f), files_i, 'UniformOutput', false);
        %fileDatenums = cellfun(@(s) s.datenum, fileInfo);
        %[~, ordFiles] = sort(fileDatenums);
        %if numel(ordFiles) > Nkeep
        %    ordFiles = ordFiles(end-Nkeep+1:end);
        %end
        %files_i = files_i(ordFiles);
...
function plot_shakeflask_70_30_results( ...
    ...
    % for j = 1:3
    %     nexttile;
    %     plot(mu_true_test(:,j), mu_pred_test(:,j), 'o', 'LineWidth', 1.2); hold on;
    %     mn = min([mu_true_test(:,j); mu_pred_test(:,j)]);
    %     mx = max([mu_true_test(:,j); mu_pred_test(:,j)]);
    %     plot([mn mx],[mn mx],'k--','LineWidth',1);
    %     axis equal; grid on;
    %     xlabel('Measured SF/T mean');
    %     ylabel('Predicted SF/T mean');
    %     title(sprintf('%s test means | RMSEP = %.3f mM', sugarNames{j}, RMSEP_test_group_perSugar(j)));
    % end
end
